    ...
    ons.notification.prompt('Insert new to-do Item label.',{
       ...
       callback: function(label){
           ...
           if(todoStorage.add(label)){
               ...
               ons.notification.alert('Failed to add item to todo list!');
           }
       }.bind(this)
    });
...
todo.refresh = function(){
    ...
    this.list.innerHTML = items.map(function(item){
        return document.querySelector('#todo-list-item').innerHTML.replace('{{label}}', item.label).replace('{{checked}}', item.status === 'completed' ? 'checked' : '');
    }).join('');
...
todo.removeItemPrompt = function(label){
    ...
    ons.notification.confirm('Are you sure to delete' + label+ 'from the todo list?', {
        title: 'Remove Item?',
